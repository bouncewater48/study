...
user.introduce(); // << 소괄호 필수! / 안녕하세요
console.log(user['address']); // key 값의 규칙성이 필요하다면 [''] 문법을 사용하여 접근할 수 있다
console.log(user.address); // 직접 접근할 수 있다
...
// 3. javascript만 출력하기(2가지 다른 방법으로, 각각 출력 타입까지)
...
// for ... out
// 이터러블(Iterable) 객체에서만 사용가능 => 배열, 문자열, Map, Set 등,... 에서 사용가능
// 순환식 객체의 값(value)를 반환한다
// 일반 객체는 이터러블 객체가 아니므로 사용 불가하다
// [Symbol.iterator] 메소드를 가진 객체를 이터러블 객체라고 한다

// 일반 객체를 for ... of로 순회하려면 배열로 변환하면 된다
// Object.keys(), Object.values(), Object.entries()를 사용한다
// Object.entries :  [key, value] 한 쌍의 배열로 반환하는 메소드
